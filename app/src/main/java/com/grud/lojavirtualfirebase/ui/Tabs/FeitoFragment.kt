...
class FeitoFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentFeitoBinding.inflate(inflater, container, false)
        return binding.root
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {//tenho registro para recupera?
                        ...
                        for (snap in snapshot.children){//percorrer todas as tarefas do banco de dado pelo ID, SALVANDO NA LISTA, QUE VAI SER ENVIADA AO ADAPTER
                            ...
                            if (task.status == 2) taskList.add(task)//comparando o statudo e add se for igual a condição
                        }
