...
import com.grud.lojavirtualfirebase.HomeFragment
...
class AfazerFragment : BaseFragment() {

    //implementação do view Binding Fragment
    private var _binding: FragmentAfazerBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentAfazerBinding.inflate(inflater, container, false)
        return binding.root
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {//tenho registro para recupera?
                        ...
                        if (task.status == 0) taskList.add(task)//comparando o statudo e add se for igual a condição
                    }
    ...
    private  fun  opcaoSelecao(task:Task, select:Int){//passando tarefa pelo task
        when(select){
